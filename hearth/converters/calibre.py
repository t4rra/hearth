...
class CalibreConverter(BaseConverter):
    ...
    def _check_calibre_installed(self) -> bool:
        ...
        if not self.ebook_convert_path:
            self._log_progress(
                "Warning: Calibre not found. Install with: brew install calibre (macOS)"
            )
            return False
        return True
    ...
    def convert(
        self, input_path: Path, output_format: ConversionFormat = ConversionFormat.MOBI
    ) -> ConversionResult:
        ...
        if not self.can_convert(input_path):
            return ConversionResult(
                False, error=f"Unsupported ebook format: {input_path.suffix}"
            )

        if not self._check_calibre_installed():
            return ConversionResult(
                False, error="Calibre ebook-convert not found on system"
            )
        if not self.ebook_convert_path:
            return ConversionResult(False, error="Calibre executable path is empty")
        ...
        try:
            ...
            calibre_path = str(self.ebook_convert_path)
            cmd = [calibre_path, str(input_path), str(output_path)]
            ...
            if result.returncode != 0:
                ...
                return ConversionResult(
                    False, error=f"Calibre conversion failed: {error_msg}"
                )

            if not output_path.exists():
                return ConversionResult(
                    False, error="Output file was not created by calibre"
                )
            ...
            return ConversionResult(
                False, error="Calibre conversion timed out after 5 minutes"
            )
        except OSError as error:
            return ConversionResult(
                False,
                error=f"Unexpected error during ebook conversion: {error}",
            )
